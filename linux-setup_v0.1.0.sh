...
set_package_manager() {
  if [[ "$DISTRIBUTION" == "Ubuntu" || "$DISTRIBUTION" == "Debian GNU/Linux" || "$DISTRIBUTION" == "Linux Mint" ]]; then
      ...
      log_message "${RED}ERROR: Unsupported distribution: $DISTRIBUTION${CLEAR}"
      exit 1
  fi
...
set_threads_count() {
  ...
  while [[ $VALID_INPUT == false ]]; do
    ...
    if [[ $NO_THREADS =~ ^[0-9]+$ ]]; then
      ...
      log_message "${RED}Invalid input. Please enter a even number less than or equal to ${CPU_THREADS}.${CLEAR_COLOR}"
    fi
  done
}
