...
# Color variables for easily switching between font styles throughout script
  BOLD='\033[1m'
  UNDERLINE='\033[4m'
  RED='\033[0;31m'
  GREEN='\033[0;32m'
...
# Assign package manager based on distribution
set_package_manager() {
  if [[ "$DISTRIBUTION" == "Ubuntu" || "$DISTRIBUTION" == "Debian GNU/Linux" || "$DISTRIBUTION" == "Linux Mint" ]]; then
      PACKAGE_MANAGER="apt-get"
  elif [[ "$DISTRIBUTION" == "Arch Linux" ]]; then
      PACKAGE_MANAGER="pacman"
  elif [[ "$DISTRIBUTION" == "CentOS Linux" || "$DISTRIBUTION" == "Fedora" ]]; then
      PACKAGE_MANAGER="yum"
  else
      log_message "Unsupported distribution: $DISTRIBUTION"
      exit 1
  fi
  SUDO_CMD=$(command -v sudo)
  if [ -n "$SUDO_CMD" ]; then
    while true; do sudo -n true; sleep 60; kill -0 "$$" || exit; done 2>/dev/null &
    PACKAGE_MANAGER="$SUDO_CMD $PACKAGE_MANAGER"
  fi
}
...
set_wallet_address() {
  ...
  if [[ $ANSWER =~ ^dero[a-z0-9]{62}$ ]]; then
    ...
    echo " "
    log_message "${RED}Alternatively, if you need one made just press ENTER without entering anything.${CLEAR_COLOR}"
    echo " "
    log_message "${RED}Try again and ensure you follow these instructions.${CLEAR_COLOR}"
    exit 1
  fi
}
...
echo " "
echo " "
log_message "{$GREEN}Script setup begun.${CLEAR_COLOR}"
set_package_manager
update_system
install_dependencies
set_platform
